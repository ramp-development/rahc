...
(() => {
  ...
  var download = () => {
    ...
    const toolkitForm = document.querySelector(".toolkit_form");
    const downloadTriggers = [...document.querySelectorAll("[data-download]")];
    const downloadPreviewTrigger = document.querySelector(
      '[data-download-modal="preview"]'
    );
    const downloadModalTrigger = document.querySelector(
      '[data-download-modal="form"]'
    );
    if (!toolkitForm || downloadTriggers.length === 0 || !downloadPreviewTrigger || !downloadModalTrigger)
      return;
    let fileURL = null;
  ...
  var preparation = () => {
    ...
    const toolkitForm = document.querySelector(".toolkit_form");
    const toolkitBody = toolkitForm?.querySelector("[data-folder-body]");
    const inputs = toolkitForm?.querySelectorAll("input");
    if (!toolkitForm || !inputs)
      return;
    ...
    levels.forEach((level) => {
      ...
      levelFolders.forEach((levelFolder) => {
        ...
        if (!body)
          return;
        const childFolders = [
          ...document.querySelectorAll(`[data-folder-parent="${item}"]`)
        ];
        childFolders.forEach((childFolder) => {
          body.append(childFolder);
        });
        const assets = [
          ...document.querySelectorAll(`[data-asset-parent="${item}"]`)
        ];
  ...
  var preview = () => {
    console.log("preview");
    const previewModalTrigger = document.querySelector(
      '[data-download-modal="preview"]'
    );
    const previewTriggers = [
      ...document.querySelectorAll('[data-preview="folder"], [data-preview="asset"]')
    ];
    const previewLinks = [...document.querySelectorAll('[data-preview="asset"]')];
    ...
    previewTriggers.forEach((trigger) => {
      ...
      trigger.addEventListener("click", (event) => {
        ...
        if (type === "folder") {
          ...
          assetLink = parent?.querySelector('[data-preview="asset"]');
        }
